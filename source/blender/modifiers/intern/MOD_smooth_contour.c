...
static float get_uv_point_overflow(BMFace *face,
                                   float uv[2],
                                   const float point_v2[2],
                                   const float mat[3][3])
{
  int vert_idx;
  float st[4][2];
...
static void get_st_point(
    BMFace *face, const float u, const float v, const float mat[3][3], float st[2])
{
  int vert_idx;
  float face_st[4][3];
  float temp_st[3];
...
static void split_BB_FF_edges_thread_start(MeshData *m_d)
{
  ...
  int edge_len;
  BMEdge **edges = BM_iter_as_arrayN(
      m_d->bm, BM_EDGES_OF_MESH, NULL, &edge_len, edge_stack, BM_DEFAULT_ITER_STACK_SIZE);

  // Do 10 samples but don't check end and start point
  float step = 1.0f / 11.0f;
  float step_arr[] = {step * 5.0f,
                      step * 6.0f,
                      step * 4.0f,
                      step * 7.0f,
                      step * 3.0f,
...
static float get_k_r(
    struct OpenSubdiv_Evaluator *eval, int face_index, float u, float v, const float cam_loc[3])
{
...
static bool check_and_shift(BMVert *vert,
                            const float new_loc[3],
                            const float new_no[3],
                            MeshData *m_d)
{
  ...
  // Will the shift create folds?
  {
    ...
    BM_ITER_ELEM (f, &iter_f, vert, BM_FACES_OF_VERT) {
      ...
      // BM_face_point_inside_test is too inaccurate to use here as some overhangs are missed with
      // it.
    ...
    BM_ITER_ELEM (f, &iter_f, vert, BM_FACES_OF_VERT) {
      ...
      BM_ITER_ELEM (e, &iter_e, f, BM_EDGES_OF_FACE) {
        if (e->v1 != vert && e->v2 != vert) {
          ...
          if (edge_c_verts == 0 && calc_if_B_nor(m_d->cam_loc, e->v1->co, e->v1->no) !=
                                       calc_if_B_nor(m_d->cam_loc, e->v2->co, e->v2->no)) {
            num_cross++;
          }
...
static void mult_face_search(
    BMFace *f, BMFace *f2, BMEdge *e, const float v1_uv[2], const float v2_uv[2], MeshData *m_d)
{
...

  {
    ...
    // Now we can begin interpolating along the edge
    {
      ...
      for (int i = 0; i < 10; i++) {
        ...
        if (!found_face) {
          // This should only happen if we shifted a border edge vert (so the interpolation path
          // lies outside the mesh faces) so just try to find the uv with the least amount of
          // overflow and use that
...
static bool bisect_search(const float v1_uv[2],
                          const float v2_uv[2],
                          BMEdge *e,
                          BMFace *orig_face,
                          float uv_result[2],
...
static void search_edge(const int i, BMEdge *e, MeshData *m_d)
{
...

  {
    ...
    if ((v1_u == 0 && v2_u == 0) || (v1_u == 1 && v2_u == 1) || (v1_v == 0 && v2_v == 0) ||
        (v1_v == 1 && v2_v == 1)) {
...
static void contour_insertion(MeshData *m_d)
{
  ...
  BM_ITER_MESH_INDEX (e, &iter_e, m_d->bm, BM_EDGES_OF_MESH, i) {
    ...
    if (calc_if_B_nor(m_d->cam_loc, e->v1->co, e->v1->no) ==
        calc_if_B_nor(m_d->cam_loc, e->v2->co, e->v2->no)) {
...
static bool cusp_triangle(struct OpenSubdiv_Evaluator *eval,
                          ...
                          Cusp *cusp)
{
  ...
  BLI_gsqueue_push(tri_que, c_tri);

  // Add this because it seems to get stuck sometimes because the triangles never becomes small
  // enough
  ...
  while (!BLI_gsqueue_is_empty(tri_que)) {
    ...
    for (int i = 0; i < 2; i++) {
      ...
      BLI_gsqueue_push(tri_que, &new_tri);
    }
  }
...
static BMFace *get_orig_face(int orig_verts,
                             BMVert *vert_arr_in[3],
                             float u_arr[3],
                             float v_arr[3],
                             float co_arr[3][3],
                             MeshData *m_d)
        ...
        if (k_r_crossing) {
          ...
          if (cusp_triangle(m_d->eval, m_d->cam_loc, face_index, &c_tri, &cusp)) {
...

            {
              ...
              // The paper suggests using a paramter space search to get the new edge intersection
              // point however in practice this seemes to be too unpresice
              ...
              // m_d->eval->evaluateLimit(m_d->eval, face_index, edge_uv[0], edge_uv[1], P, du,
              // dv);

              {
                ...
                for (int i = 0; i < 10; i++) {
                  ...
                  m_d->eval->evaluateLimit(
                      m_d->eval, face_index, edge_uv[0], edge_uv[1], P, du, dv);
              ...
              float cusp_dist_to_edge1 = dist_to_line_v3(
                  cusp.cusp_co, cusp_e_vert->co, edge->v1->co);
              float cusp_dist_to_edge2 = dist_to_line_v3(
                  cusp.cusp_co, cusp_e_vert->co, edge->v2->co);
              ...
              if (cusp_dist_to_edge1 < BM_edge_calc_length(edge) * 0.2f ||
                ...
                BM_ITER_ELEM (cusp_edge, &iter_e, f, BM_EDGES_OF_FACE) {
                  if (cusp_edge != edge &&
                      (cusp_edge->v1 == edge_vert || cusp_edge->v2 == edge_vert)) {
                    // Found edge
                    break;
                  }
                }
              ...
              if (edge_idx < orig_edges) {
                ...
                if ((v1_idx + 1) > orig_verts) {
                  Vert_buf v_buf_old = BLI_buffer_at(
                      m_d->new_vert_buffer, Vert_buf, v1_idx - orig_verts);
                  v_buf.orig_edge = v_buf_old.orig_edge;
                  v_buf.orig_face = orig_face;
                }
                else if ((v2_idx + 1) > orig_verts) {
                  Vert_buf v_buf_old = BLI_buffer_at(
                      m_d->new_vert_buffer, Vert_buf, v2_idx - orig_verts);
                  v_buf.orig_edge = v_buf_old.orig_edge;
                  v_buf.orig_face = orig_face;
                }
              }
...
static void cusp_insertion(MeshData *m_d)
{
  ...
  for (cusp_i = 0; cusp_i < m_d->cusp_edges->count; cusp_i++) {
    ...
    if (len_v3v3(cusp.cusp_co, cusp.cusp_e->v1->co) < BM_edge_calc_length(cusp.cusp_e) * 0.2f) {
      if (BM_vert_edge_count(cusp.cusp_e->v1) == 4 &&
          check_and_shift(cusp.cusp_e->v1, cusp.cusp_co, cusp.cusp_no, m_d)) {
    ...
    else if (len_v3v3(cusp.cusp_co, cusp.cusp_e->v2->co) <
             BM_edge_calc_length(cusp.cusp_e) * 0.2f) {
      if (BM_vert_edge_count(cusp.cusp_e->v2) == 4 &&
          check_and_shift(cusp.cusp_e->v2, cusp.cusp_co, cusp.cusp_no, m_d)) {
        float uv_P[2] = {cusp.u, cusp.v};
...
static bool poke_and_move(BMFace *f,
                          const float new_pos[3],
                          const float du[3],
                          const float dv[3],
                          Radi_vert *r_vert,
    ...
    */
    ...
    do {
      ...
      f_new = BM_face_create_quad_tri(
          bm, l_iter->v, l_iter->next->v, vert, NULL, f, BM_CREATE_NOP);
...
static void mult_radi_search(BLI_Buffer *diff_f,
                             const float mat[3][3],
                             const float edge1_mid[2],
                             const float edge2_mid[2],
                             const float val_1,
                             const float val_2,
                             bool is_B,
                             const float rad_plane_no[3],
                             BMVert *C_vert,
                             BMFace *poke_face,
                             MeshData *m_d)
{
  ...
  BM_ITER_ELEM (vert, &iter_v, poke_face, BM_VERTS_OF_FACE) {
    // Take the mean normal as face normal. It should represent the surface normal of the surface
    // better
      ...
      for (edge_i = 0; edge_i < edge_count; edge_i++) {
...

        {
          ...
          // This way, we can dissolve edge_vert and it will leave a nice triangle face
          {
            ...
            while (((cur_loop = BM_vert_step_fan_loop(cur_loop, &cur_e)) != first_loop) &&
                   (cur_loop != NULL)) {
            ...
            if (edge_idx != flip_edges) {
              ...
              for (; edge_idx <= op_idx; op_idx--) {
                ...
                if (edge_idx == op_idx) {
                  ...
                  // check_degenerate is too strict in this case (in most cases the face area will
                  // be near zero)
  ...
  for (int vert_i = 0; vert_i < m_d->radi_vert_buffer->count; vert_i++) {
    ...
    // Begin extenting the radi edge
    {
      ...
      for (int i = 0; i < 11; i++) {
        ...
        m_d->eval->evaluateLimit(
            m_d->eval, BM_elem_index_get(cur_face), uv_P[0], uv_P[1], P, du, dv);
...
static void create_fan_copy(
    BMesh *bm_copy, BMFace *input_face, GHash *vhash, GHash *ehash, GHash *fhash)
{
    ...
    BM_ITER_ELEM (vert, &iter_v, inface->face, BM_VERTS_OF_FACE) {
...

      {
        ...
        BM_ITER_ELEM_INDEX (f, &iter_f, vert, BM_FACES_OF_VERT, face_idx) {
          ...
          face_area[face_idx] = area_tri_v2(
              store_2d[face_idx][0], store_2d[face_idx][1], store_2d[face_idx][2]);
        ...
        mul_v3_fl(cent, 1.0f / (float)BM_vert_edge_count(vert));
        {
          ...
          for (face_idx = 0; face_idx < face_count; face_idx++) {
            ...
            for (int i = 0; i < samples; i++) {
              ...
              BLI_rng_get_tri_sample_float_v2(rng,
                                              store_2d[face_idx][0],
                                              store_2d[face_idx][1],
                                              store_2d[face_idx][2],
                                              cur_v2);
              ...
              for (int f_idx = 0; f_idx < orig_faces.count; f_idx++) {
                ...
                if (point_inside_v2(mat, cur_v2, uv_face)) {
                  ...
                  m_d->eval->evaluateLimit(
                      m_d->eval, BM_elem_index_get(uv_face), uv_P[0], uv_P[1], P, du, dv);
  ...
  // 4. Edge Splitting
  {
    ...
    for (face_i = 0; face_i < initial_inco_len; face_i++) {
      ...
      BM_ITER_ELEM_INDEX (edge, &iter_e, inface->face, BM_EDGES_OF_FACE, edge_idx) {
        ...
        c_radi_verts += is_vert_in_buffer(edge->v1, m_d->C_verts) +
                        is_vert_in_buffer(edge->v2, m_d->C_verts);
        ...
        for (int i = 0; i < 10; i++) {
          ...
          for (int f_idx = 0; f_idx < orig_faces.count; f_idx++) {
            ...
            if (point_inside_v2(mat, cur_v2, f)) {
              ...
              m_d->eval->evaluateLimit(
                  m_d->eval, BM_elem_index_get(f), uv_P[0], uv_P[1], P, du, dv);
      ...
      }
      {
        ...
        BM_ITER_ELEM_INDEX (edge, &iter_e, copy_f, BM_EDGES_OF_FACE, edge_idx) {
          ...
          for (int j = 0; j < 10; j++) {
            ...
            if (split_vert == NULL) {
              split_vert = split_edge_and_move_nor(NULL,
                                                   bm_temp,
                                                   temp_e,
                                                   split_data[edge_idx][j].cusp_co,
                                                   split_data[edge_idx][j].cusp_no);
            }
            else {
        ...
        if (done) {
          ...
          BM_ITER_ELEM_INDEX (edge, &iter_e, inface->face, BM_EDGES_OF_FACE, edge_idx) {
            if (edge_idx == best_edge) {
              ...
              split_vert = split_edge_and_move_nor(
                  m_d, m_d->bm, edge, best_edge_split.cusp_co, best_edge_split.cusp_no);
...
static struct OpenSubdiv_Evaluator *create_osd_eval(BMesh *bm, Mesh *mesh)
{
  ...
  osd_evaluator->setCoarsePositions(osd_evaluator, vert_array, 0, no_of_verts);
...
static void create_vert_mapping(MeshData *m_d)
{
  ...
  // When this function is called, bm is just a copy of bm_orig. So we can now map the original
  // verts together.
...
/* bmesh only function */
static Mesh *mybmesh_do(Mesh *mesh, SmoothContourModifierData *mmd, float cam_loc[3])
{
  ...
  if (mmd->osd_eval == NULL) {
    ...
    // TODO make a smarter cache system. Or even better, resuse the OSD data from a subsurf
    // modifier!
  ...
  if (mmd->flag & MOD_SMOOTHCONTOUR_TRIANG) {
  ...
  }
  {
    ...
    if (mmd->flag & MOD_SMOOTHCONTOUR_SEL) {
...
{
  // TODO it does depend on normals. But they have to be the actual geometry normals...
  // So return true or false here?
  return false;
}
...
static void panel_draw(const bContext *C, Panel *panel)
{
  ...
  uiItemR(layout, &ptr, "do_tri", 0, NULL, ICON_NONE);
  uiItemR(layout, &ptr, "do_ff_bb_split", 0, NULL, ICON_NONE);
  uiItemR(layout, &ptr, "do_cusp_dect", 0, NULL, ICON_NONE);

  uiItemR(layout, &ptr, "do_insert", 0, NULL, ICON_NONE);
  uiItemR(layout, &ptr, "do_cusp_insert", 0, NULL, ICON_NONE);
  uiItemR(layout, &ptr, "do_rad_insert", 0, NULL, ICON_NONE);

  uiItemR(layout, &ptr, "do_rad_flip", 0, NULL, ICON_NONE);
  uiItemR(layout, &ptr, "do_opti", 0, NULL, ICON_NONE);

  uiItemR(layout, &ptr, "camera_object", 0, NULL, ICON_NONE);
  uiItemR(layout, &ptr, "do_sel", 0, NULL, ICON_NONE);
...
{
  modifier_panel_register(region_type, eModifierType_SmoothContour, panel_draw);
}

ModifierTypeInfo modifierType_SmoothContour = {
    /* name */ "SmoothContour",
    /* structName */ "SmoothContourModifierData",
    /* structSize */ sizeof(SmoothContourModifierData),
